...
public class EmailService {
    ...
    private Properties loadConfig() {
        ...
        try (InputStream is = getClass().getResourceAsStream(CONFIG_PATH)) {
            ...
            System.err.println("[EmailService] Impossible de charger la configuration: " + e.getMessage());
        }
        return props;
    }
    ...
    public boolean sendEmail(String toEmail, String subject, String htmlBody) {
        if (!enabled) {
            System.out.println("[EmailService] Email désactivé. Sujet: " + subject);
            return false;
        }
        if (senderEmail.isEmpty() || senderPassword.isEmpty()) {
            System.err.println("[EmailService] Configuration SMTP manquante (email/mot de passe).");
            return false;
        }
        if (toEmail == null || toEmail.isBlank()) {
            System.err.println("[EmailService] Adresse destinataire vide.");
            return false;
        }
        ...
        try {
            ...
            System.out.println("[EmailService] ✅ Email envoyé à " + toEmail + " — " + subject);
            return true;

        } catch (Exception e) {
            System.err.println("[EmailService] ❌ Erreur d'envoi: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
