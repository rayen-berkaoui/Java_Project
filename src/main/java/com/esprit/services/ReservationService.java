...
public class ReservationService {
    ...
    // =====================================================
    // GET CASH RESERVATIONS FOR PARTENAIRE'S ETABLISSEMENTS
    // =====================================================
    public List<Reservation> getCashReservationsForPartenaire(int partenaireId) {
        List<Reservation> list = new ArrayList<>();
        String sql = "SELECT r.*, p.type_service, p.nb_personnes, p.id_etablissement, p.id_client, e.nom AS nom_etablissement " +
                     "FROM reservation r " +
                     "JOIN panier p ON r.id_panier = p.id_panier " +
                     "JOIN etablissement e ON p.id_etablissement = e.id_etablissement " +
                     "WHERE e.id_partenaire = ? AND r.mode_paiement = 'Especes' AND r.statut_paiement = 'En cours de paiement' " +
                     "ORDER BY r.date_paiement DESC";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ...
            while (rs.next()) {
                Reservation rr = mapReservation(rs);
                try { rr.setIdClient(rs.getInt("id_client")); } catch (SQLException ignored) {}
                try { rr.setNomEtablissement(rs.getString("nom_etablissement")); } catch (SQLException ignored) {}
                list.add(rr);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public int countCashReservationsForPartenaire(int partenaireId) {
