 ...
 *   2. Open link in browser → user pays via Flouci app
...
public class FlouciPaymentService {
    ...
    // Redirect URLs (localhost for desktop app)
    private static final String ACCEPT_URL = "https://tabaani.tn/payment/success";
    private static final String DECLINE_URL = "https://tabaani.tn/payment/decline";
     ...
     * @return PaymentResult with payment link to open in browser
     */
    public PaymentResult generatePayment(double amountInDT, String description) {
        try {
            ...
            body.addProperty("accept_url", ACCEPT_URL);
            body.addProperty("decline_url", DECLINE_URL);
            ...
            if (responseCode == 200) {
                ...
                if (link != null && paymentId != null) {
                    return new PaymentResult(true,
                        "Lien de paiement genere! Ouvrez-le pour payer.",
    ...
    public PaymentResult verifyPayment(String paymentId) {
        try {
            HttpURLConnection conn = (HttpURLConnection) new URL(VERIFY_URL + paymentId).openConnection();
