...
public class PdfExportService {
    ...
    public static void exportEtablissement(Etablissement etab, File outputFile) throws Exception {
        ...
        doc.add(new LineSeparator(new SolidLine(3)).setStrokeColor(GOLD).setMarginBottom(20));
        doc.add(new Paragraph("FICHE ÉTABLISSEMENT")
                .setFont(bold).setFontSize(22).setFontColor(GOLD)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(20));

        Table info = new Table(UnitValue.createPercentArray(new float[]{30, 70})).useAllAvailableWidth();
        info.setBackgroundColor(new DeviceRgb(248, 248, 248));
        addInfoRow(info, bold, regular, "Nom", safeStr(etab.getNom()));
        addInfoRow(info, bold, regular, "Type", safeStr(etab.getType()));
        addInfoRow(info, bold, regular, "Ville", safeStr(etab.getVille()));
        addInfoRow(info, bold, regular, "Adresse", safeStr(etab.getAdresse()));
        addInfoRow(info, bold, regular, "Téléphone", safeStr(etab.getTelephone()));
        addInfoRow(info, bold, regular, "Email", safeStr(etab.getEmail()));
        addInfoRow(info, bold, regular, "Gamme Prix", safeStr(etab.getGammePrix()));
        addInfoRow(info, bold, regular, "Horaires", safeStr(etab.getHoraires()));
        doc.add(info);

        if (etab.getDescription() != null && !etab.getDescription().isBlank()) {
            doc.add(new Paragraph("\nDescription").setFont(bold).setFontSize(14).setFontColor(GOLD).setMarginTop(16));
            doc.add(new Paragraph(etab.getDescription()).setFont(regular).setFontSize(10));
        }

        // QR Code
        try {
            BufferedImage qrImage = QRCodeService.generateEtabQR(etab);
            if (qrImage != null) {
                ByteArrayOutputStream baos = new ByteArrayOutputStream();
                javax.imageio.ImageIO.write(qrImage, "PNG", baos);
                ImageData qrData = ImageDataFactory.create(baos.toByteArray());
                doc.add(new Paragraph("\nQR Code").setFont(bold).setFontSize(14).setFontColor(GOLD).setMarginTop(16));
                doc.add(new Image(qrData).setMaxWidth(180).setMaxHeight(180)
                        .setHorizontalAlignment(HorizontalAlignment.CENTER));
                doc.add(new Paragraph("Scannez pour localiser cet établissement")
                        .setFont(regular).setFontSize(8).setFontColor(MUTED_TEXT).setTextAlignment(TextAlignment.CENTER));
            }
        } catch (Exception qrEx) {
            System.err.println("QR generation skipped: " + qrEx.getMessage());
        }

        doc.add(new Paragraph("\nGénéré le " + LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))
                .setFont(regular).setFontSize(8).setFontColor(MUTED_TEXT).setTextAlignment(TextAlignment.CENTER).setMarginTop(30));
        doc.close();
    }
    ...
    public static void exportActivite(Activite act, File outputFile) throws Exception {
        ...
        doc.add(new LineSeparator(new SolidLine(3)).setStrokeColor(GOLD).setMarginBottom(20));
        doc.add(new Paragraph("FICHE ACTIVITÉ")
                .setFont(bold).setFontSize(22).setFontColor(GOLD)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(20));

        Table info = new Table(UnitValue.createPercentArray(new float[]{30, 70})).useAllAvailableWidth();
        info.setBackgroundColor(new DeviceRgb(248, 248, 248));
        addInfoRow(info, bold, regular, "Nom", safeStr(act.getNomActivite()));
        addInfoRow(info, bold, regular, "Catégorie", safeStr(act.getCategorie()));
        addInfoRow(info, bold, regular, "Niveau", safeStr(act.getNiveau()));
        addInfoRow(info, bold, regular, "Durée", act.getDuree() != null ? act.getDuree() + " min" : "—");
        addInfoRow(info, bold, regular, "Prix", act.getPrix() != null ? act.getPrix().toPlainString() + " " + safeStr(act.getDevise()) : "—");
        addInfoRow(info, bold, regular, "Places", act.getNbPlaces() != null ? String.valueOf(act.getNbPlaces()) : "—");
        addInfoRow(info, bold, regular, "Âge min", act.getAgeMin() != null ? String.valueOf(act.getAgeMin()) : "—");
        addInfoRow(info, bold, regular, "Adresse départ", safeStr(act.getAdresseDepart()));
        addInfoRow(info, bold, regular, "Date début", act.getDateDebut() != null ? act.getDateDebut().toString() : "—");
        addInfoRow(info, bold, regular, "Date fin", act.getDateFin() != null ? act.getDateFin().toString() : "—");
        doc.add(info);

        if (act.getDescription() != null && !act.getDescription().isBlank()) {
            doc.add(new Paragraph("\nDescription").setFont(bold).setFontSize(14).setFontColor(GOLD).setMarginTop(16));
            doc.add(new Paragraph(act.getDescription()).setFont(regular).setFontSize(10));
        }

        if (act.getEquipementInclus() != null && !act.getEquipementInclus().isBlank()) {
            doc.add(new Paragraph("\nÉquipement inclus").setFont(bold).setFontSize(14).setFontColor(GOLD).setMarginTop(10));
            doc.add(new Paragraph(act.getEquipementInclus()).setFont(regular).setFontSize(10));
        }

        if (act.getConditionsAnnulation() != null && !act.getConditionsAnnulation().isBlank()) {
            doc.add(new Paragraph("\nConditions d'annulation").setFont(bold).setFontSize(14).setFontColor(GOLD).setMarginTop(10));
            doc.add(new Paragraph(act.getConditionsAnnulation()).setFont(regular).setFontSize(10));
        }

        // QR Code
        try {
            BufferedImage qrImage = QRCodeService.generateActiviteQR(act);
            if (qrImage != null) {
                ByteArrayOutputStream baos = new ByteArrayOutputStream();
                javax.imageio.ImageIO.write(qrImage, "PNG", baos);
                ImageData qrData = ImageDataFactory.create(baos.toByteArray());
                ...
                doc.add(new Image(qrData).setMaxWidth(180).setMaxHeight(180)
                        .setHorizontalAlignment(HorizontalAlignment.CENTER));
                doc.add(new Paragraph("Scannez pour les details de cette activité")
                        .setFont(regular).setFontSize(8).setFontColor(MUTED_TEXT).setTextAlignment(TextAlignment.CENTER));
            }
        } catch (Exception qrEx) {
            System.err.println("QR generation skipped: " + qrEx.getMessage());
    ...
    public static void exportAllEtablissements(ArrayList<Etablissement> etablissements, File outputFile) throws Exception {
        PdfWriter writer = new PdfWriter(outputFile);
        PdfDocument pdf = new PdfDocument(writer);
        Document doc = new Document(pdf, PageSize.A4);
        ...
        doc.add(new Paragraph("CATALOGUE DES ÉTABLISSEMENTS")
                .setFont(bold).setFontSize(20).setFontColor(GOLD)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(10));
        doc.add(new Paragraph(etablissements.size() + " établissements — " + LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))
                .setFont(regular).setFontSize(10).setFontColor(MUTED_TEXT)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(20));

        Table table = new Table(UnitValue.createPercentArray(new float[]{5, 25, 15, 15, 15, 25})).useAllAvailableWidth();
        String[] headers = {"#", "Nom", "Type", "Ville", "Gamme", "Contact"};
        for (String h : headers) {
        ...
        for (Etablissement e : etablissements) {
            table.addCell(new Cell().add(new Paragraph(String.valueOf(idx++)).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(e.getNom())).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(e.getType())).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(e.getVille())).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(e.getGammePrix())).setFont(regular).setFontSize(8)).setPadding(4));
            String contact = safeStr(e.getTelephone());
            if (!safeStr(e.getEmail()).equals("—")) contact += "\n" + e.getEmail();
            table.addCell(new Cell().add(new Paragraph(contact).setFont(regular).setFontSize(7)).setPadding(4));
        }
        doc.add(table);
        doc.close();
    }

    public static void exportAllActivites(ArrayList<Activite> activites, File outputFile) throws Exception {
        PdfWriter writer = new PdfWriter(outputFile);
        PdfDocument pdf = new PdfDocument(writer);
        Document doc = new Document(pdf, PageSize.A4);
        doc.setMargins(30, 35, 30, 35);
        ...
        doc.add(new Paragraph("CATALOGUE DES ACTIVITÉS")
                .setFont(bold).setFontSize(20).setFontColor(GOLD)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(10));
        doc.add(new Paragraph(activites.size() + " activités — " + LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))
                .setFont(regular).setFontSize(10).setFontColor(MUTED_TEXT)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(20));

        Table table = new Table(UnitValue.createPercentArray(new float[]{5, 22, 14, 12, 12, 14, 21})).useAllAvailableWidth();
        String[] headers = {"#", "Nom", "Catégorie", "Niveau", "Durée", "Prix", "Adresse départ"};
        for (String h : headers) {
            table.addHeaderCell(new Cell()
                    .add(new Paragraph(h).setFont(bold).setFontSize(9).setFontColor(ColorConstants.WHITE))
                    .setBackgroundColor(new DeviceRgb(45, 45, 60))
        ...
        for (Activite a : activites) {
            table.addCell(new Cell().add(new Paragraph(String.valueOf(num++)).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(a.getNomActivite())).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(a.getCategorie())).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(a.getNiveau())).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(a.getDuree() != null ? a.getDuree() + " min" : "—").setFont(regular).setFontSize(8)).setPadding(4));
            String prix = a.getPrix() != null ? a.getPrix().stripTrailingZeros().toPlainString() + " " + safeStr(a.getDevise()) : "—";
            table.addCell(new Cell().add(new Paragraph(prix).setFont(regular).setFontSize(8)).setPadding(4));
            table.addCell(new Cell().add(new Paragraph(safeStr(a.getAdresseDepart())).setFont(regular).setFontSize(8)).setPadding(4));
        }
        doc.add(table);
        doc.close();
    }

    private static void addInfoRow(Table table, PdfFont bold, PdfFont regular, String label, String value) {
        table.addCell(new Cell().add(new Paragraph(label).setFont(bold).setFontSize(10).setFontColor(GOLD))
