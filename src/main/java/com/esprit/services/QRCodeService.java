...
public class QRCodeService {
    ...
    /**
     * Generate a QR code BufferedImage for an Etablissement.
     */
    public static BufferedImage generateEtabQR(Etablissement etab) throws WriterException {
        StringBuilder sb = new StringBuilder();
        if (etab.getLatitude() != null && etab.getLongitude() != null
                && etab.getLatitude() != 0 && etab.getLongitude() != 0) {
            sb.append("https://www.google.com/maps?q=")
              .append(String.format(java.util.Locale.US, "%.6f", etab.getLatitude()))
              .append(",")
              .append(String.format(java.util.Locale.US, "%.6f", etab.getLongitude()));
            sb.append("\n\n");
        }
        sb.append(etab.getNom() != null ? etab.getNom() : "Etablissement");
        if (etab.getVille() != null) sb.append("\n").append(etab.getVille());
        if (etab.getTelephone() != null) sb.append("\nTel: ").append(etab.getTelephone());
        if (etab.getEmail() != null) sb.append("\n").append(etab.getEmail());
        if (etab.getGammePrix() != null) sb.append("\nPrix: ").append(etab.getGammePrix());
        return createQRImage(sb.toString(), QR_SIZE);
    }

    /**
     * Generate a QR code BufferedImage for an Activite.
     */
    public static BufferedImage generateActiviteQR(Activite act) throws WriterException {
        StringBuilder sb = new StringBuilder();
        sb.append(act.getNomActivite() != null ? act.getNomActivite() : "Activite");
        if (act.getCategorie() != null) sb.append("\nCategorie: ").append(act.getCategorie());
        if (act.getAdresseDepart() != null) sb.append("\nDepart: ").append(act.getAdresseDepart());
        if (act.getPrix() != null) sb.append("\nPrix: ").append(act.getPrix().toPlainString()).append(" ").append(act.getDevise() != null ? act.getDevise() : "DT");
        if (act.getNiveau() != null) sb.append("\nNiveau: ").append(act.getNiveau());
        return createQRImage(sb.toString(), QR_SIZE);
    }
}
