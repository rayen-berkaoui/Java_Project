...
public class utilisateurServices implements ICrud<utilisateur> {
    ...
    private utilisateur mapUser(ResultSet rs) throws SQLException {
        ...
        try { u.setTotpEnabled(rs.getBoolean("totp_enabled")); } catch (SQLException ignored) {}
        try { u.setThemePreference(rs.getString("theme_preference")); } catch (SQLException ignored) {}
        try { u.setLanguage(rs.getString("language")); } catch (SQLException ignored) {}
        try { u.setLoyaltyPoints(rs.getInt("loyalty_points")); } catch (SQLException ignored) {}
        Date date = rs.getDate("date_creation");
    ...
    public boolean saveLanguagePreference(int userId, String language) {
        String sql = "UPDATE utilisateur SET language = ? WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    // =====================================================
    // PARTENAIRE REGISTRATION (EN_ATTENTE status)
    // =====================================================
    public boolean registerPartenaire(String nom, String prenom, String email,
                                      String password, int numTel, String nfcId) {
        ...
        try {
            ...
            ps.setString(4, password);
            ps.setInt(5, numTel);
            ps.setString(6, nfcId);
            ps.executeUpdate();

            ResultSet keys = ps.getGeneratedKeys();
            if (keys.next()) {
                int userId = keys.getInt(1);
                String sqlReq = "INSERT INTO partenaire_requests (user_id, status) VALUES (?, 'EN_ATTENTE')";
                PreparedStatement ps2 = con.prepareStatement(sqlReq);
                ps2.setInt(1, userId);
                ps2.executeUpdate();
            }
            return true;
    ...
    // =====================================================
    // CHECK IF USER IS EN_ATTENTE
    // =====================================================
    public boolean isUserEnAttente(String email, String password) {
        String sql = "SELECT statut FROM utilisateur WHERE email = ? AND mot_de_passe = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, email);
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return "EN_ATTENTE".equalsIgnoreCase(rs.getString("statut"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // =====================================================
    // GET PENDING PARTENAIRE REQUESTS (for admin)
    // =====================================================
    public List<PartenaireRequest> getPendingPartenaireRequests() {
        List<PartenaireRequest> list = new ArrayList<>();
        String sql = "SELECT * FROM partenaire_requests WHERE status = 'EN_ATTENTE' ORDER BY request_date DESC";
        try {
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery(sql);
            while (rs.next()) {
                PartenaireRequest pr = new PartenaireRequest();
                pr.setId(rs.getInt("id"));
                pr.setUserId(rs.getInt("user_id"));
                pr.setStatus(rs.getString("status"));
                Timestamp ts = rs.getTimestamp("request_date");
                if (ts != null) pr.setRequestDate(ts.toLocalDateTime());
                list.add(pr);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    // APPROVE PARTENAIRE
    // =====================================================
    public boolean approvePartenaire(int requestId, int adminId) {
        try {
            // Get user_id from request
            String sqlGet = "SELECT user_id FROM partenaire_requests WHERE id = ?";
            ...
            String sqlReq = "UPDATE partenaire_requests SET status='ACCEPTE', response_date=NOW(), admin_id=? WHERE id=?";
            ...
            // Activate user
            String sqlUser = "UPDATE utilisateur SET statut='ACTIF' WHERE id=?";
    ...
    // REJECT PARTENAIRE
    // =====================================================
    public boolean rejectPartenaire(int requestId, int adminId) {
        try {
            String sqlGet = "SELECT user_id FROM partenaire_requests WHERE id = ?";
            PreparedStatement psGet = con.prepareStatement(sqlGet);
            psGet.setInt(1, requestId);
            ...
            String sqlReq = "UPDATE partenaire_requests SET status='REFUSE', response_date=NOW(), admin_id=? WHERE id=?";
            PreparedStatement psReq = con.prepareStatement(sqlReq);
            psReq.setInt(1, adminId);
            psReq.setInt(2, requestId);
            psReq.executeUpdate();

            String sqlUser = "UPDATE utilisateur SET statut='BLOQUE' WHERE id=?";
            PreparedStatement psUser = con.prepareStatement(sqlUser);
            psUser.setInt(1, userId);
            psUser.executeUpdate();
    ...
    // =====================================================
    // COUNT PENDING PARTENAIRE REQUESTS
    // =====================================================
    public int countPendingPartenaires() {
        String sql = "SELECT COUNT(*) FROM partenaire_requests WHERE status = 'EN_ATTENTE'";
        try {
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery(sql);
            if (rs.next()) return rs.getInt(1);
    ...
    // =====================================================
    public boolean isPartenaire(int roleId) {
        String sql = "SELECT nom FROM role WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, roleId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return "PARTENAIRE".equalsIgnoreCase(rs.getString("nom"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
