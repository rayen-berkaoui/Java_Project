...
public class ChatbotService {
    ...
    private static final String SYSTEM_PROMPT = """
        Tu es "Tabaani AI", un assistant de voyage intelligent et sympathique pour l'application TABAANI SmartTravel.
    ...
    /**
     * Attempt to call the Gemini API. Returns null on any failure.
     */
    private String tryApiCall(String userMessage) {
        try {
            ...
            // API returned an error — remove the user message we added
            conversationHistory.remove(conversationHistory.size() - 1);
            return null;

        } catch (Exception e) {
            // Remove the user message since API failed
