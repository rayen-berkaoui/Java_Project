...
import com.esprit.entities.ForumUserWarning;
import com.esprit.utils.MyDataBase;

import java.sql.*;
import java.time.LocalDateTime;

public class ForumWarningService {

    private static final int BASE_BLOCK_HOURS = 24;
    private final Connection con;
    private final ForumEmailService emailService;

    public ForumWarningService() {
        this.con = MyDataBase.getInstance().getConnection();
        this.emailService = new ForumEmailService();
        ensureTablesExist();
    }
    ...
    private void ensureTablesExist() {
        try (Statement st = con.createStatement()) {
            ...
            System.err.println("[ForumWarningService] Tables check: " + e.getMessage());
        }
    }

    public ForumUserWarning getOrCreate(String userKey, String userEmail) throws SQLException {
        String selectSql = "SELECT * FROM user_warnings WHERE user_key = ?";
        try (PreparedStatement ps = con.prepareStatement(selectSql)) {
            ps.setString(1, userKey);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ForumUserWarning uw = mapFromResultSet(rs);
        ...
        try (PreparedStatement ps = con.prepareStatement(insertSql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                ForumUserWarning uw = new ForumUserWarning(userKey, userEmail);
    ...
    public WarningResult processViolation(String userKey, String userEmail, String badWord, String postContent) {
        try {
            ForumUserWarning uw = getOrCreate(userKey, userEmail);
            ...
            System.out.println("[ForumWarningService] " + userKey + " → " + action +
            ...
            System.err.println("[ForumWarningService] Erreur SQL: " + e.getMessage());
    ...
    private void logWarningHistory(String userKey, int level, String badWord, String postContent, String action, boolean emailSent) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            System.err.println("[ForumWarningService] History log error: " + e.getMessage());
        }
    }

    private ForumUserWarning mapFromResultSet(ResultSet rs) throws SQLException {
        ForumUserWarning uw = new ForumUserWarning();
