...
import com.esprit.entities.PartenaireRequest;
...
public class DashboardController {
    ...
    @FXML private Label drawerUserNameLabel;
    ...
    @FXML private VBox partenaireApprovalPane;
    @FXML private Label pendingPartenairesLabel;
    @FXML private VBox partenaireRequestsContainer;
    @FXML private Button btnPartenaireApproval;

    // Tableau panels (fx:include)
    @FXML private Node panelTableauEtab;
    @FXML private Node panelTableauActivite;
    @FXML private Node panelForumAdmin;
    @FXML private Node panelDashboardTourisme;
    ...
    @FXML private Button btnTableauEtab;
    @FXML private Button btnTableauActivite;
    @FXML private Button btnForumAdmin;
    @FXML private Button btnDashboardTourisme;
    ...
    private void collectStaticLabels() {
        if (staticLabelsCollected || contentArea == null) return;
        contentArea.getScene().getRoot().lookupAll(".sidebar-button, .sidebar-button-active").forEach(n -> {
            if (n instanceof Labeled l && l.getText() != null && !l.getText().isBlank())
                staticOriginals.put(l, l.getText());
        });
        contentArea.getScene().getRoot().lookupAll("Label").forEach(n -> {
            if (n instanceof Label l && l.getText() != null && !l.getText().isBlank()
                    && !staticOriginals.containsKey(l) && l.getText().length() > 1)
                staticOriginals.put(l, l.getText());
        });
    ...
    public void setUserName(String userName) {
        ...
        // Populate drawer
        if (drawerUserNameLabel != null) drawerUserNameLabel.setText(userName);
        if (drawerInitials != null && userName != null && !userName.isEmpty()) {
        ...
        if (drawerUserRole != null) drawerUserRole.setText("Administrateur");
    }
    ...
    private void loadAdminReservations(Integer filterUserId, String filterStatus) {
        ...
        for (Reservation r : allRes) {
            ...
            if ("En cours de paiement".equalsIgnoreCase(sp) || "En attente".equalsIgnoreCase(sp)) pendingCount++;
    ...
    private VBox buildResCard(Reservation r, DateTimeFormatter dtf, Integer filterUserId, String filterStatus) {
        ...
        else if ("En attente".equalsIgnoreCase(statut)) { statutColor = "#FFD700"; statutBg = "rgba(255,215,0,0.1)"; }
        else if ("Paye".equalsIgnoreCase(statut) || "Pay\u00e9".equalsIgnoreCase(statut)) { statutColor = "#51CF66"; statutBg = "rgba(81,207,102,0.1)"; }
        else if ("Confirme".equalsIgnoreCase(statut)) { statutColor = "#51CF66"; statutBg = "rgba(81,207,102,0.1)"; }
        else if ("Rembours\u00e9".equalsIgnoreCase(statut) || "Annulee".equalsIgnoreCase(statut)) { statutColor = "#FF6B6B"; statutBg = "rgba(255,107,107,0.1)"; }
        ...
        // Cash payment approval is handled by the partenaire owner, not admin

        // Restaurant/cafe reservations: admin can confirm or reject
        if (isRestoRes && "En attente".equalsIgnoreCase(statut)) {
            Button confirmBtn = new Button("\u2705 Confirmer");
            confirmBtn.setStyle("-fx-background-color: linear-gradient(to right, #2d7a3a, #1a5e28); -fx-text-fill: white; -fx-font-size: 10; -fx-padding: 5 14; -fx-background-radius: 8; -fx-cursor: hand; -fx-font-weight: bold;");
            confirmBtn.setOnAction(e -> {
                Alert confirm = createConfirmDialog("Confirmer la reservation ?",
                    "Reservation #" + r.getIdReservation() + " - " + clientName + "\n" + (r.getNomEtablissement() != null ? r.getNomEtablissement() : "") + "\n" + r.getNbPersonnes() + " personne(s)\n\nLa reservation sera confirmee.");
                Optional<ButtonType> result = confirm.showAndWait();
                if (result.isPresent() && result.get() == ButtonType.OK) {
                    reservationService.updateStatut(r.getIdReservation(), "Confirme");
                    auditService.log(currentAdminId, currentUserName, "CONFIRM_RESERVATION", "RESERVATION", r.getIdReservation(),
                        "Reservation #" + r.getIdReservation() + " (" + clientName + ")", "En attente", "Confirme");
            ...
            rejectBtn.setOnAction(e -> {
                Alert confirm = createConfirmDialog("Rejeter la reservation ?",
                    "La reservation #" + r.getIdReservation() + " sera annulee.");
                Optional<ButtonType> result = confirm.showAndWait();
                if (result.isPresent() && result.get() == ButtonType.OK) {
                    reservationService.updateStatut(r.getIdReservation(), "Annulee");
                    auditService.log(currentAdminId, currentUserName, "REJECT_RESERVATION", "RESERVATION", r.getIdReservation(),
                        "Reservation #" + r.getIdReservation() + " (" + clientName + ")", "En attente", "Annulee");
                    loadAdminReservations(filterUserId, filterStatus);
                }
            });
    ...
    private void setActiveNav(Button active) {
        Button[] navButtons = {btnDashboard, btnCharts, btnUsers, btnAudit, btnPermissions, btnSettings, btnSupport, btnPanierAdmin, btnReservationAdmin, btnTableauEtab, btnTableauActivite, btnForumAdmin, btnDashboardTourisme, btnPartenaireApproval};
    ...
    @FXML private void handleShowDashboard() { closeDrawer(); setActiveNav(btnDashboard); loadDashboardData(); showPane(dashboardPane); }
    @FXML private void handleShowCharts() { closeDrawer(); setActiveNav(btnCharts); loadCharts(); showPane(chartsPane); }
    @FXML private void handleShowUsers() { closeDrawer(); setActiveNav(btnUsers); loadUsersTable(); showPane(usersPane); }
    @FXML private void handleShowAudit() { closeDrawer(); setActiveNav(btnAudit); loadAuditTrail(); showPane(auditPane); }
    @FXML private void handleShowPermissions() { closeDrawer(); setActiveNav(btnPermissions); loadPermissionsMatrix(); showPane(permissionsPane); }
    @FXML private void handleShowSettings() { closeDrawer(); setActiveNav(btnSettings); showPane(settingsPane); }
    @FXML private void handleShowSupport() { closeDrawer(); setActiveNav(btnSupport); showPane(supportPane); }
    @FXML private void handleShowTableauEtab() { closeDrawer(); setActiveNav(btnTableauEtab); showPaneNode(panelTableauEtab); }
    @FXML private void handleShowTableauActivite() { closeDrawer(); setActiveNav(btnTableauActivite); showPaneNode(panelTableauActivite); }
    @FXML private void handleShowForumAdmin() { closeDrawer(); setActiveNav(btnForumAdmin); showPaneNode(panelForumAdmin); }
    @FXML private void handleShowDashboardTourisme() { closeDrawer(); setActiveNav(btnDashboardTourisme); showPaneNode(panelDashboardTourisme); }
    @FXML private void handleShowPartenaireApproval() { closeDrawer(); setActiveNav(btnPartenaireApproval); loadPartenaireRequests(); showPane(partenaireApprovalPane); }

    @FXML public void handleRefreshDashboard() { loadDashboardData(); }

    private void hideAllPanels() {
        VBox[] allPanes = {dashboardPane, chartsPane, usersPane, auditPane, permissionsPane, settingsPane, supportPane, panierAdminPane, reservationAdminPane, partenaireApprovalPane};
        for (VBox pane : allPanes) {
            if (pane != null) pane.setVisible(false);
        }
        Node[] includePanels = {panelTableauEtab, panelTableauActivite, panelForumAdmin, panelDashboardTourisme};
        for (Node n : includePanels) {
            if (n != null) n.setVisible(false);
        }
    }

    private void showPaneNode(Node paneToShow) {
        hideAllPanels();
        if (paneToShow != null) {
            ...
            slide.setFromX(20); slide.setToX(0); slide.setInterpolator(Interpolator.SPLINE(0.25, 0.1, 0.25, 1));
            new ParallelTransition(fade, slide).play();
        }
    }
    ...
    // PARTENAIRE APPROVAL
    // =====================================================
    private void loadPartenaireRequests() {
        if (partenaireRequestsContainer == null) return;
        partenaireRequestsContainer.getChildren().clear();

        java.util.List<PartenaireRequest> requests = userService.getPendingPartenaireRequests();
        if (pendingPartenairesLabel != null) pendingPartenairesLabel.setText(String.valueOf(requests.size()));

        if (requests.isEmpty()) {
            Label empty = new Label("\u2705 Aucune demande en attente");
            empty.setStyle("-fx-text-fill: #51CF66; -fx-font-size: 14;");
            partenaireRequestsContainer.getChildren().add(empty);
            return;
        }

        EmailService emailService = new EmailService();

        for (PartenaireRequest req : requests) {
            // Get user info
            utilisateur user = userService.getUserById(req.getUserId());
            String name = user != null ? user.getPrenom() + " " + user.getNom() : "Utilisateur #" + req.getUserId();
            ...
            titleLabel.setStyle("-fx-text-fill: #FFD700; -fx-font-size: 14; -fx-font-weight: bold;");

            Label emailLabel = new Label("Email: " + email);
            emailLabel.setStyle("-fx-text-fill: #aaa; -fx-font-size: 11;");

            String dateStr = req.getRequestDate() != null ? req.getRequestDate().toLocalDate().toString() : "N/A";
            Label dateLabel = new Label("Demande le: " + dateStr);
            dateLabel.setStyle("-fx-text-fill: #666; -fx-font-size: 10;");

            info.getChildren().addAll(titleLabel, emailLabel, dateLabel);
            ...
            approveBtn.setOnAction(e -> {
                userService.approvePartenaire(req.getId(), currentAdminId);
                if (user != null) {
                    new Thread(() -> emailService.sendPartenaireApprovedEmail(user.getEmail(), user.getPrenom() + " " + user.getNom())).start();
                }
                loadPartenaireRequests();
            });
            ...
            rejectBtn.setStyle("-fx-background-color: #C0392B; -fx-text-fill: white; -fx-font-size: 11; " +
                    "-fx-padding: 8 16; -fx-background-radius: 8; -fx-cursor: hand;");
            rejectBtn.setOnAction(e -> {
                userService.rejectPartenaire(req.getId(), currentAdminId);
                if (user != null) {
                    new Thread(() -> emailService.sendPartenaireRejectedEmail(user.getEmail(), user.getPrenom() + " " + user.getNom())).start();
                }
                loadPartenaireRequests();
            });
            ...
            partenaireRequestsContainer.getChildren().add(card);
        }
    }
}