...
public class ReservationController {
    ...
    private VBox buildReservationCard(Reservation r, int index) {
        ...
        if (isCash && isPaid && !isRestoRes) {
            Button upgradeBtn = new Button("\uD83D\uDCB3 Passer en Paymee");
    ...
    // ================================================================
    // UPGRADE CASH -> PAYMEE (IN-APP WEBVIEW)
    // ================================================================
    private void showUpgradeToCardDialog(Reservation r) {
        Stage paymentStage = new Stage();
        paymentStage.setTitle("Passer en Paiement Paymee");
        ...
        com.esprit.services.PaymeePaymentService paymeeSvc = new com.esprit.services.PaymeePaymentService();
        ...
        Label topTitle = new Label("Especes \u2192 Paymee");
        ...
        Label payTitle = new Label("Passer de Especes a Paymee");
        ...
        Label infoSec = new Label("\uD83D\uDD12 Paiement securise via Paymee");
        ...
        Button payBtn = new Button("\uD83D\uDCB3 Payer via Paymee - " + String.format("%.2f DT", r.getMontantTotal()));
        ...
        webEngine.locationProperty().addListener((obs, oldUrl, newUrl) -> {
            if (newUrl != null) {
                System.out.println("[PAYMEE UPGRADE WEBVIEW] " + newUrl);
                String lower = newUrl.toLowerCase();
                if (lower.contains(com.esprit.services.PaymeePaymentService.SUCCESS_URL.toLowerCase()) || lower.contains("/payment/success")) {
                    ...
                    if (paymentIdHolder[0] != null) {
                        new Thread(() -> {
                            ...
                            com.esprit.services.PaymeePaymentService.PaymentResult verifyResult = paymeeSvc.verifyPayment(paymentIdHolder[0]);
                            javafx.application.Platform.runLater(() -> {
                                if (verifyResult.isSuccess()) {
                                    processUpgradeSuccess(r, pointsToEarn);
                                    paymentStage.close();
                                } else {
                                    showUpgradeVerifyScreen(paymentStage, root, r, pointsToEarn, paymentIdHolder[0], paymeeSvc);
                                }
                            });
                        }).start();
                    }
                } else if (lower.contains(com.esprit.services.PaymeePaymentService.FAIL_URL.toLowerCase()) || lower.contains("/payment/fail")) {
                    javafx.application.Platform.runLater(() -> {
        ...
        payBtn.setOnAction(e -> {
            ...
            new Thread(() -> {
                com.esprit.services.PaymeePaymentService.PaymentResult result = paymeeSvc.initPayment(
            ...
            if (currentUser != null && currentUser.getEmail() != null) {
                new Thread(() -> {
                    ...
                    emailService.sendPaymentConfirmationEmail(currentUser.getEmail(), name,
                        ...
                        "Paymee (upgrade)", pointsToEarn, totalPts);
                }).start();
            }
            showMessage("\u2705 Paiement Paymee confirme ! +" + pointsToEarn + " points gagnes \u2B50", true);
            loadData();
        }
    }
    ...
    private void showUpgradeVerifyScreen(Stage paymentStage, BorderPane root, Reservation r,
                                          int pointsToEarn, String paymeePaymentId,
                                          com.esprit.services.PaymeePaymentService paymeeSvc) {
        ...
        verifyBtn.setOnAction(e -> {
            ...
            new Thread(() -> {
                com.esprit.services.PaymeePaymentService.PaymentResult result = paymeeSvc.verifyPayment(paymeePaymentId);
                javafx.application.Platform.runLater(() -> {
                    if (result.isSuccess()) {
                        processUpgradeSuccess(r, pointsToEarn);
                        new Thread(() -> {
    ...
    private void showInAppCalendar(Reservation r) {
        ...
        // Calendar month navigation - start on CURRENT month so user sees today
        ...
        todayBtn.setStyle("-fx-background-color: rgba(100,181,246,0.2); -fx-text-fill: #64B5F6; -fx-padding: 6 14; -fx-background-radius: 8; -fx-cursor: hand; -fx-font-weight: bold; -fx-font-size: 10;");
        monthNav.getChildren().addAll(prevMonth, monthLabel, todayBtn, nextMonth);
        ...
        Runnable buildCalendar = () -> {
            ...
            for (int day = 1; day <= daysInMonth; day++) {
                ...
                } else if (isToday && hasReservation) {
                    style += "-fx-background-color: rgba(81,207,102,0.25); -fx-text-fill: #51CF66; -fx-border-color: #64B5F6; -fx-border-width: 2; -fx-border-radius: 8;";
                } else if (isToday) {
                    style += "-fx-background-color: linear-gradient(to bottom, rgba(100,181,246,0.35), rgba(100,181,246,0.15)); -fx-text-fill: #FFFFFF; -fx-border-color: #64B5F6; -fx-border-width: 2; -fx-border-radius: 8;";
        ...
        todayBtn.setOnAction(e -> { currentMonth[0] = java.time.YearMonth.now(); buildCalendar.run(); eventDetails.setVisible(false); });
