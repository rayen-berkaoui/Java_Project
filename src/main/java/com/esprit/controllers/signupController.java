...
public class signupController {
    ...
    private void completeRegistration() {
        boolean success;
        boolean isPartenaire = (pendingRoleId == 2);

        if (isPartenaire) {
            success = utilisateurService.registerPartenaire(
                    pendingFirstName,
                    pendingLastName,
                    pendingEmail,
                    pendingPassword,
            ...
            success = utilisateurService.registerUser(
                    pendingFirstName,
                    pendingLastName,
                    pendingEmail,
                    pendingPassword,
    ...
    private void showPartenaireEnAttente() {
        errorLabel.setStyle("-fx-text-fill: #F39C12; -fx-font-size: 13;");
        errorLabel.setText("⏳ Votre demande partenaire a été envoyée.\nUn administrateur examinera votre demande.\nVous recevrez un email une fois approuvé.");

        // Send notification email to admin
        new Thread(() -> {
            try {
                emailService.sendPartenaireRequestNotification(pendingEmail, pendingFirstName + " " + pendingLastName);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }).start();
